...
function handleUnauthorized() {
  ...
  // Show toast notification ONLY if not already on login page
  const isOnLoginPage = window.location.pathname === '/login'
  const toastStore = window.__TOAST_STORE__
  if (toastStore && !isOnLoginPage) {
  ...
  try {
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        // FIX: Ajout timeout pour éviter blocage UI
        ...
        try {
          ...
          if (fetchError.name === 'AbortError') {
            throw new ApiError('Requête timeout (30s)', 408, {})
          }
          throw fetchError
        }
      } catch (error) {
