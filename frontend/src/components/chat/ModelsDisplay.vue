  ...
  <div class="models-display bg-gray-900/50 rounded-xl p-4 border border-gray-700/50">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
      <div class="bg-gray-800/50 rounded-lg p-3 text-center border border-gray-700/30">
        <div class="text-2xl font-bold text-primary-400">{{ totalModels }}</div>
    ...
    <!-- Footer -->
    <div class="mt-4 pt-3 border-t border-gray-700/50 flex items-center justify-between text-xs text-gray-500">
...
const categories = computed(() => {
  ...
  for (const model of props.models) {
    ...
    else if (name.includes('embed') || name.includes('nomic') || name.includes('bge') || 
             name.includes('mxbai') || name.includes('rerank')) {
