<template>
  <div class="h-full flex flex-col bg-gray-900/50 border-l border-gray-700/50">
    <!-- Header -->
    <div class="p-3 border-b border-gray-700/50 flex items-center justify-between bg-gradient-to-r from-gray-800/50 to-transparent">
      ...
      <div class="flex items-center gap-2">
        <span v-if="run?.id" class="text-xs text-gray-500 font-mono bg-gray-800/50 px-2 py-0.5 rounded">
          #{{ run.id.slice(0, 8) }}
        </span>
        <span
    ...
    <div v-if="chat.wsState !== 'connected'" class="px-3 py-2 bg-red-500/10 border-b border-red-500/30 flex items-center gap-2">
    ...
    <div v-if="!run" class="flex-1 flex items-center justify-center text-gray-500 text-sm p-4 text-center">
      <div>
        <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br from-primary-500/20 to-purple-600/20 flex items-center justify-center">
    ...
    <div v-else class="flex-1 overflow-y-auto flex flex-col">
      ...
      <div class="p-4 border-b border-gray-700/30 bg-gradient-to-b from-gray-800/30 to-transparent">
        <div class="flex items-center justify-between text-xs mb-3">
          ...
          <div class="flex items-center gap-2">
            <span v-if="run.repairCycles > 0" class="px-2 py-0.5 rounded-full bg-orange-500/20 text-orange-300 text-[10px]">
              {{ run.repairCycles }} repair{{ run.repairCycles > 1 ? 's' : '' }}
            </span>
            <span v-if="run.duration" class="text-gray-400 font-mono">{{ formatDuration(run.duration) }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between">
          ...
          >
            ...
            <span
              ...
              :class="isPhaseActive(phase) ? 'text-primary-400' : isPhaseComplete(phase) ? 'text-green-400' : 'text-gray-500'"
            >
              {{ getPhaseLabel(phase) }}
            </span>
        ...
        >
          <span class="flex items-center justify-center gap-1.5">
            ...
            <span
              ...
              :class="activeTab === tab.id ? 'bg-primary-500/30 text-primary-300' : 'bg-gray-700 text-gray-400'"
            >
              {{ tab.badge }}
            </span>
          </span>
      ...
      <div class="flex-1 overflow-y-auto p-3">
        ...
        <div v-if="activeTab === 'tools'" class="space-y-2">
          <div v-if="run.toolCalls.length === 0" class="text-gray-500 text-sm text-center py-8">
          ...
          >
            <div class="flex items-center gap-2 mb-2">
              <div class="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500/30 to-blue-600/20 flex items-center justify-center">
              ...
              <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-700/50 text-gray-400">Iter {{ tool.iteration }}</span>
            </div>
            <div v-if="chat.settings.showToolParams && tool.params" class="mt-2">
              <pre class="text-xs text-gray-400 bg-gray-900/50 rounded-lg p-2 overflow-x-auto border border-gray-700/30">{{ formatParams(tool.params) }}</pre>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'thinking'" class="space-y-2">
          <div v-if="run.thinking.length === 0" class="text-gray-500 text-sm text-center py-8">
            <svg class="w-10 h-10 mx-auto mb-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="text-xs text-purple-300 font-medium">{{ thought.phase || 'Reflexion' }}</span>
        ...
        <div v-if="activeTab === 'verification'" class="space-y-3">
          <div v-if="!run.verification && run.verificationItems.length === 0" class="text-gray-500 text-sm text-center py-8">
            <svg class="w-10 h-10 mx-auto mb-2 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-for="item in run.verificationItems" :key="item.check_name" class="bg-gray-800/50 rounded-lg p-3 border transition-colors" :class="getVerificationItemClass(item)">
            <div class="flex items-center gap-2">
              <div
              ...
              >
                <svg v-if="item.status === 'running'" class="w-3.5 h-3.5 text-yellow-400 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                </svg>
                <svg v-else-if="item.status === 'passed'" class="w-3.5 h-3.5 text-green-400" fill="currentColor" viewBox="0 0 20 20">
              ...
              <span class="text-sm font-medium" :class="item.status === 'passed' ? 'text-green-300' : item.status === 'failed' ? 'text-red-300' : 'text-yellow-300'">
                {{ item.check_name }}
              </span>
            </div>
            <div v-if="item.output" class="mt-2">
              <pre class="text-xs text-gray-400 bg-gray-900/50 rounded-lg p-2 overflow-x-auto max-h-24 border border-gray-700/30">{{ item.output }}</pre>
            </div>
          ...
          <!-- Verdict Final -->
          <div v-if="run.verdict" class="mt-4 p-4 rounded-xl border" :class="run.verdict.status === 'PASS' ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'">
            <div class="flex items-center gap-3 mb-3">
              <div
                class="w-10 h-10 rounded-xl flex items-center justify-center"
                :class="run.verdict.status === 'PASS' ? 'bg-green-500/20' : 'bg-red-500/20'"
              >
                <svg v-if="run.verdict.status === 'PASS'" class="w-6 h-6 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <span class="font-semibold text-lg" :class="run.verdict.status === 'PASS' ? 'text-green-300' : 'text-red-300'">
                ...
                <div class="flex items-center gap-2 mt-0.5">
                  ...
                  <span class="text-xs text-gray-400">{{ Math.round(run.verdict.confidence * 100) }}% confiance</span>
                </div>
              </div>
            </div>
            <div v-if="run.verdict.issues?.length" class="mt-3 pt-3 border-t border-gray-700/30">
              ...
              <ul class="space-y-1">
                <li v-for="issue in run.verdict.issues" :key="issue" class="flex items-start gap-2 text-sm text-red-300">
        ...
        <div v-if="activeTab === 'raw'" class="space-y-2">
          ...
          <pre class="text-xs text-gray-400 bg-gray-900/70 rounded-lg p-3 overflow-x-auto max-h-96 border border-gray-700/30 font-mono">{{ formatRaw() }}</pre>
        </div>
      </div>
    </div>

    <!-- Footer with actions -->
    <div v-if="run?.complete" class="p-3 border-t border-gray-700/50 bg-gradient-to-t from-gray-800/30 to-transparent space-y-2">
      <!-- Verdict badge -->
      <div v-if="run.verdict" class="flex items-center justify-between mb-2 px-1">
        ...
        <span
          ...
          :class="run.verdict.status === 'PASS' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'"
        >
          {{ run.verdict.status }}
        </span>
      </div>
...
const ThinkingIcon = () => h('svg', { fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
...
const VerificationIcon = () => h('svg', { fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'stroke-width': '2', d: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' })
])

const RawIcon = () => h('svg', { fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'stroke-width': '2', d: 'M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4' })
])

const tabs = computed(() => [
  { id: 'tools', label: 'Tools', badge: run.value?.toolCalls?.length || 0, icon: ToolsIcon },
  { id: 'thinking', label: 'Thinking', badge: run.value?.thinking?.length || 0, icon: ThinkingIcon },
...
const verdictClass = computed(() => {
  ...
  switch (run.value.workflowPhase) {
    case 'spec': return 'bg-purple-500/20 text-purple-300'
    case 'plan': return 'bg-blue-500/20 text-blue-300'
    case 'execute': return 'bg-yellow-500/20 text-yellow-300'
    case 'verify': return 'bg-cyan-500/20 text-cyan-300'
    case 'repair': return 'bg-orange-500/20 text-orange-300'
...
function getPhaseIcon(phase) {
  const icons = {
    spec: () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'stroke-width': '2', d: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' })
    ]),
    plan: () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'stroke-width': '2', d: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' })
    ]),
    execute: () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'stroke-width': '2', d: 'M13 10V3L4 14h7v7l9-11h-7z' })
    ]),
    verify: () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'stroke-width': '2', d: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' })
    ]),
    repair: () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
    ...
    complete: () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
...
function getPhaseClass(phase) {
  ...
  if (isPhaseActive(phase)) return 'bg-primary-500/20 text-primary-400 border border-primary-500/50 ring-2 ring-primary-500/30 animate-pulse'
