...
export const PhaseTimeouts = Object.freeze({
  ...
  execute: 120000,    // 2min for execution (CRQ-2026-0203-001: increased from 90s)
  verify: 120000,    // 2min for verification
  repair: 120000,    // 2min for repair
  default: 120000,    // Default timeout (CRQ-2026-0203-001: increased from 90s)
})
...
export function updatePhaseStatus(run, phase, status, meta = {}) {
  ...
  if ((status === PhaseStatus.COMPLETE || status === PhaseStatus.SKIPPED || status === PhaseStatus.FAILED) && !run.phases[phase].endedAt) {
