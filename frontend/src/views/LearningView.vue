  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 p-6">
    <div class="max-w-7xl mx-auto">
      ...
      <div v-else-if="error" class="bg-red-900/20 border border-red-500/50 rounded-xl p-6 mb-8">
        ...
        <button @click="loadData" class="mt-4 px-4 py-2 bg-red-600 hover:bg-red-500 rounded-lg text-white">
          Réessayer
        </button>
      </div>
      ...
      <div v-else>
        ...
        <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6 mb-8">
          ...
          <div v-if="topPatterns.length === 0" class="text-center py-12 text-gray-500">
            ...
            <p class="text-xs mt-2">Les patterns seront créés automatiquement lors des interactions</p>
          </div>
        ...
        <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-gray-800/50 rounded-lg p-4">
              ...
              <div class="text-2xl font-bold text-white">{{ learningStats?.experiences_count || 0 }}</div>
            </div>
            <div class="bg-gray-800/50 rounded-lg p-4">
              <div class="text-xs text-gray-400 mb-1">Patterns</div>
              <div class="text-2xl font-bold text-white">{{ learningStats?.patterns_count || 0 }}</div>
            </div>
            <div class="bg-gray-800/50 rounded-lg p-4">
              <div class="text-xs text-gray-400 mb-1">Corrections</div>
              <div class="text-2xl font-bold text-white">{{ learningStats?.corrections_count || 0 }}</div>
            </div>
            <div class="bg-gray-800/50 rounded-lg p-4">
              <div class="text-xs text-gray-400 mb-1">État ChromaDB</div>
              <div class="text-sm font-bold" :class="learningStats?.status === 'connected' ? 'text-green-400' : 'text-red-400'">
...
async function loadPatterns() {
  try {
    ...
    topPatterns.value = response.data.patterns || []
  } catch (err) {
