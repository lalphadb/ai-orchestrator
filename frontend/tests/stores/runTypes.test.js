...
describe('PhaseTimeouts', () => {
  it('has timeout for each phase', () => {
    ...
    expect(PhaseTimeouts.execute).toBe(90000)
    expect(PhaseTimeouts.verify).toBe(120000)
    expect(PhaseTimeouts.repair).toBe(120000)
    expect(PhaseTimeouts.default).toBe(90000)
  })
...
describe('createRunState (prompt compliance)', () => {
  ...
  it('has thinking[] (v8 name, was thinkingLog)', () => {
    const run = createRunState('run-123')
    expect(Array.isArray(run.thinking)).toBe(true)
    expect(run.thinking.length).toBe(0)
  })

  it('has tools[] (v8 name, was toolCalls)', () => {
    const run = createRunState('run-123')
    expect(Array.isArray(run.tools)).toBe(true)
  })

  it('has verification[] (v8 name, was verificationItems)', () => {
    const run = createRunState('run-123')
    expect(Array.isArray(run.verification)).toBe(true)
  })

  it('has startedAt/lastEventAt/endedAt as ISO strings', () => {
    const before = new Date().toISOString()
    const run = createRunState('run-123')
    const after = new Date().toISOString()

    // V8: timestamps are ISO strings
  ...
  it('has terminal: false initially (v8: boolean, not null)', () => {
    const run = createRunState('run-123')
    expect(run.terminal).toBe(false)
  })

  it('has watchdog: timerId + timeoutMs + lastHeartbeatAt', () => {
    const run = createRunState('run-123')
    ...
    expect(typeof run.watchdog.lastHeartbeatAt).toBe('string')
  })

  it('has v8 properties (no legacy getter aliases)', () => {
    const run = createRunState('run-123', { conversationId: 'conv-1' })

    // V8 properties exist
    expect(run.conversation_id).toBe('conv-1')
    expect(run.startedAt).toBeDefined()
    expect(run.lastEventAt).toBeDefined()
    expect(run.endedAt).toBeNull()
    expect(Array.isArray(run.thinking)).toBe(true)
    expect(Array.isArray(run.tools)).toBe(true)
    expect(Array.isArray(run.verification)).toBe(true)
...
describe('getPhaseTimeout', () => {
  ...
  it('returns default timeout for unknown phase', () => {
    expect(getPhaseTimeout('unknown')).toBe(90000)
    expect(getPhaseTimeout(null)).toBe(90000)
  })
})
