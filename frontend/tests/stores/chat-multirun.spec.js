 ...
 * Updated for V8 normalized property names
 */
...
describe('Chat Store - Phase 2: Multi-Run Support', () => {
  ...
  describe('Run Registry', () => {
    ...
    it('marks run as complete', () => {
      ...
      expect(run.endedAt).toBeTruthy() // V8: ISO string
    ...
    it('cleans up old completed runs', async () => {
      const chat = useChatStore()
      ...
      const { setTerminal } = await import('@/stores/runTypes')
  ...
  describe('✓ Playbook Test 2: Déconnecter/reconnecter le WS', () => {
    it('preserves run state across WS reconnection', () => {
      ...
      run.tools.push({ tool: 'bash', timestamp: Date.now() }) // V8: tools not toolCalls
  ...
  describe('✓ Playbook Test 3: Timeout sans event → run FAILED', () => {
    it('watchdog marks stuck run as FAILED after timeout', () => {
      const chat = useChatStore()
      ...
      const run = chat.runs.get(runId)
      expect(run.status).toBe('running')
      expect(run.watchdog.timerId).toBeTruthy() // V8: watchdog.timerId

      // Simulate no events for >90s by setting lastEventAt to old timestamp
      run.lastEventAt = new Date(Date.now() - 95000).toISOString() // V8: ISO string
  ...
  describe('✓ Playbook Test 5: Aucun crash console navigateur', () => {
    it('handles invalid events gracefully without crashing', () => {
      ...
      // markRunComplete with unknown run_id creates placeholder (V8 behavior)
      // This should not throw
      expect(() => chat.markRunComplete('unknown-run', { response: 'Done' })).not.toThrow()

      // markRunFailed with unknown run_id should not throw
      expect(() => chat.markRunFailed('unknown-run-2', 'Error')).not.toThrow()
  ...
  describe('Per-Run Watchdog', () => {
    ...
    it('updates lastEventAt on events', () => {
      const chat = useChatStore()
      ...
      const run = chat.runs.get(runId)
      const initialTime = run.lastEventAt // V8: ISO string

      // Advance time and simulate event
      vi.advanceTimersByTime(5000)
      const newTime = new Date(Date.now()).toISOString()
      run.lastEventAt = newTime
      run.watchdog.lastHeartbeatAt = newTime

      expect(run.lastEventAt > initialTime).toBe(true) // ISO strings compare correctly

      // Watchdog should not fail if events keep coming
      vi.advanceTimersByTime(15000) // Check interval
      expect(run.status).toBe('running')

      // But should fail if no events for >90s (v8 default timeout)
      const oldTime = new Date(Date.now() - 95000).toISOString()
      run.lastEventAt = oldTime
      run.watchdog.lastHeartbeatAt = oldTime
  ...
  describe('Out-of-Order Event Handling', () => {
    it('creates placeholder run for unknown run_id', () => {
      ...
      // V8: getOrCreateRun creates a placeholder with null messageId
      const run = chat.getOrCreateRun(runId)

      expect(chat.runs.has(runId)).toBe(true)
      expect(run.messageId).toBe(null) // Placeholder has no message
      expect(run.assistantMessageId).toBe(null)
    })
    ...
    it('completes placeholder when conversation_created arrives', () => {
      ...
      const run = chat.getOrCreateRun(runId)
      expect(run.messageId).toBe(null) // Placeholder indicator

      // Add some events to placeholder
      run.thinking.push({ message: 'Early thinking' })
      run.tools.push({ tool: 'bash' }) // V8: tools not toolCalls
      ...
      expect(run.conversation_id).toBe('conv-123')
      expect(run.thinking.length).toBe(1)
      expect(run.tools.length).toBe(1) // V8: tools not toolCalls
    })
  })
  ...
  describe('exportRunReport', () => {
    it('exports active run report as JSON', () => {
      ...
      const run = chat.runs.get('run-123')
      run.tools.push({ tool: 'bash', params: {}, iteration: 1 }) // V8: tools not toolCalls
