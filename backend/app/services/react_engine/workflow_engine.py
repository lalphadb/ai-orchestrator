        ...
        try:
            ...
            if is_simple or skip_spec:
                ...
                await self._send_phase(
                    websocket, "execute", workflow_id, message="Traitement de la demande..."
                )

                execution = await self._execute(user_message, model, history, websocket)
                ...
                await self._send_phase(
                    websocket, "spec", workflow_id, message="Analyse et spécification..."
                )
                ...
                await self._send_phase(websocket, "plan", workflow_id, message="Planification...")
                ...
                state.phase = WorkflowPhase.EXECUTE
                await self._send_phase(
                    websocket, "execute", workflow_id, message="Exécution du plan..."
                )

                # Enrichir le prompt avec le contexte du plan
                enriched_message = self._enrich_with_plan(user_message, spec, plan)

                execution = await self._execute(enriched_message, model, history, websocket)
                state.execution = execution
                ...
                if self.verify_required:
                    ...
                    await self._send_phase(
                        websocket, "verify", workflow_id, message="Vérification QA..."
                    )

                    verification = await self._run_verification(spec, websocket, workflow_id)
            ...
            await self._send_ws(websocket, "error", {"message": str(e), "phase": state.phase.value})
...
---"""
    ...
    async def _execute(
        ...
        result = await react_engine.run(
            user_message=message, model=model, history=history, websocket=websocket
        )
    ...
    async def run_qa_checks(
        ...
        for check_name, tool_name, params in qa_checks:
            ...
            await self._send_verification_item(websocket, run_id, check_name, "running")
            ...
            await self._send_verification_item(
                websocket,
                run_id,
                check_name,
    ...
    async def _send_ws(self, ws: Optional[WebSocket], msg_type: str, data: Any, run_id: str = None):
    ...
    async def _send_phase(self, ws: Optional[WebSocket], phase: str, run_id: str, **extra):
        """Envoie un événement de changement de phase"""
        await self._send_ws(
            ws, "phase", {"phase": phase, "status": "in_progress", **extra}, run_id=run_id
        )

    async def _send_verification_item(
        self,
        ...
        """Envoie un événement pour un item de vérification"""
        await self._send_ws(
            ws,
            "verification_item",
            {
