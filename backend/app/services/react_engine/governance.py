...
class GovernanceManager:
    ...
    def classify_action(self, tool_name: str, params: Dict) -> ActionCategory:
        ...
        read_tools = {
            "read_file", "list_directory", "search_files", "search_directory",
            "get_system_info", "get_datetime", "get_audit_log",
            "git_status", "git_diff", "list_llm_models"
        }
        
        # Actions SAFE (sûres)
        safe_tools = {
            "calculate", "http_request"  # GET only
        }
        
        # Actions MODERATE (modérées)
        moderate_tools = {
            "execute_command",  # Dépend de la commande
            "run_tests", "run_lint", "run_format", "run_typecheck"
        }
        ...
        if tool_name == "write_file":
            ...
            if os.path.exists(path):
                ...
                backup_path = os.path.join(
                    self.backup_dir,
                    f"{action_id}_{os.path.basename(path)}.backup"
                )
                try:
        ...
        try:
            if rollback_info.rollback_type == "file_restore":
                ...
                if rollback_info.rollback_command:
                    from .secure_executor import secure_executor, ExecutionRole
                    result = await secure_executor.execute(
                        rollback_info.rollback_command,
                        role=ExecutionRole.ADMIN,
                        timeout=30
                    )
                    if result.success:
                        logger.info(f"[GOVERNANCE] Rollback commande: {rollback_info.rollback_command}")
    ...
    def get_pending_verifications(self) -> List[Dict]:
        ...
        pending = [
            ...
            if a.verification_required and not a.verified
        ]
        return [
            {
                "action_id": a.action_id,
