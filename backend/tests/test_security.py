...
from app.services.react_engine.tools import (BUILTIN_TOOLS, fail,
                                             is_command_allowed,
                                             is_path_in_workspace, ok)
...
class TestAllowlist:
    ...
    def test_allowed_commands_pass(self):
        ...
        # Ces commandes sont autorisées pour le rôle VIEWER (lecture seule)
        viewer_allowed = ["ls", "cat", "grep", "df -h"]
        for cmd in viewer_allowed:
            ...
            assert result is True, f"{cmd} devrait être autorisée en VIEWER"

        # git/docker/systemctl nécessitent le rôle OPERATOR
        operator_allowed = ["git status", "docker ps", "systemctl status nginx"]
        for cmd in operator_allowed:
            parse_ok, argv, _ = executor._parse_command_safe(cmd)
            assert parse_ok, f"{cmd} devrait parser"
            result, _ = executor._is_command_allowed(argv, ExecutionRole.OPERATOR)
            assert result is True, f"{cmd} devrait être autorisée en OPERATOR"
...
class TestWorkspaceIsolation:
    """Tests de l'isolation du workspace avec workspace temporaire"""

    def test_path_in_workspace_valid(self, tmp_path, monkeypatch):
        """Les chemins dans le workspace sont valides"""
        # Utiliser un workspace temporaire pour le test
    ...
    def test_path_outside_workspace_invalid(self, tmp_path, monkeypatch):
    ...
    def test_relative_paths_resolved_from_cwd(self, tmp_path, monkeypatch):
        ...
        if isinstance(r3, tuple):
            assert r3[0] is True, f"{workspace}/test.py devrait être dans le workspace"
