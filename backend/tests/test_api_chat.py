...
class TestChatAPI:
    ...
    def test_chat_allows_anonymous(self, client: TestClient):
        """Test que /chat accepte les requêtes anonymes (get_current_user_optional)"""
        response = client.post("/api/v1/chat", json={"message": "quelle heure?"})
        # L'endpoint accepte les requêtes anonymes depuis SCEN-8
        assert response.status_code == 200

    def test_chat_with_invalid_token(self, client: TestClient):
        """Test chat avec token invalide (traité comme anonyme avec get_current_user_optional)"""
        response = client.post(
            "/api/v1/chat",
            json={"message": "Hello"},
            headers={"Authorization": "Bearer invalid_token"},
        )
        # Depuis SCEN-8, token invalide = utilisateur anonyme, pas d'erreur 401
        assert response.status_code == 200
...
class TestToolsAPI:
    ...
    def test_list_tools_public(self, client: TestClient):
        """Test liste outils accessible sans auth (SCEN-8: get_current_user_optional)"""
        response = client.get("/api/v1/tools")
        # Endpoint public depuis SCEN-8 pour permettre aux utilisateurs de voir les outils
        assert response.status_code == 200
