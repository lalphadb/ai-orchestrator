...
class TestFilesystemTools:
    """Tests des outils filesystem avec workspace temporaire"""

    @pytest.mark.asyncio
    async def test_read_file_in_workspace(self, tmp_path, monkeypatch):
        """read_file fonctionne dans le workspace"""
        from app.core.config import settings

        # Utiliser workspace temporaire
        workspace = str(tmp_path / "workspace")
        Path(workspace).mkdir(exist_ok=True)
        monkeypatch.setattr(settings, "WORKSPACE_DIR", workspace)

        test_file = Path(workspace) / "test_read.txt"
    ...
    @pytest.mark.asyncio
    async def test_write_file_in_workspace(self, tmp_path, monkeypatch):
        ...
        test_file = Path(workspace) / "test_write.txt"

        result = await BUILTIN_TOOLS.execute(
            "write_file",
            path=str(test_file),
            content="Test content",
            justification="Test filesystem tool",
        )
        assert result["success"] is True
...
class TestSearchDirectory:
    """Tests de l'outil search_directory (v6.2) avec workspace temporaire"""
    ...
    def test_search_directory_in_workspace(self, tmp_path, monkeypatch):
        ...
        test_dir = Path(workspace) / "test_search_dir"
        test_dir.mkdir(exist_ok=True)

        # Appeler directement la fonction
        result = search_directory(name="test_search_dir", base=workspace)
        assert result["success"] is True
        assert result["data"]["count"] >= 1

        test_dir.rmdir()

    def test_search_directory_base_not_allowed(self, tmp_path, monkeypatch):
        """search_directory refuse les bases non autorisées"""
        from app.core.config import settings
        from app.services.react_engine import tools as tools_module
        from app.services.react_engine.tools import search_directory
        ...
        # Patcher SEARCH_ALLOWED_BASES pour exclure /etc
        monkeypatch.setattr(tools_module, "SEARCH_ALLOWED_BASES", [workspace])

        # Appeler directement la fonction avec base hors workspace
