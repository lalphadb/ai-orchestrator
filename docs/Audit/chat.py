...
async def handle_rerun_verify(data: dict, ws: WebSocket, db: Session):
    ...
    Ne ré-exécute pas les outils, juste les checks QA.
    """
    conversation_id = data.get("conversation_id")
    ...
    await ws.send_json({
        "type": "phase",
        "data": {"phase": "verify", "status": "running", "message": "Re-vérification en cours..."}
    })
    
    try:
        # Récupérer les outils utilisés
        tools_used_names = json.loads(last_response.tools_used or "[]")
        tools_used = [{"tool": t, "output": {}} for t in tools_used_names]
        ...
        # Obtenir nouveau verdict via le verifier
        verdict = await verifier_service.judge(
            original_request="(re-verification)",
            spec=None,
            execution_result={"response": last_response.content, "tools_used": tools_used_names},
        ...
        await ws.send_json({
            ...
            "data": {
                "verification": verification.model_dump() if hasattr(verification, 'model_dump') else verification,
                "verdict": verdict.model_dump() if hasattr(verdict, 'model_dump') else verdict,
                "action": "rerun_verify",
                "message": f"Re-vérification terminée: {verdict.status if hasattr(verdict, 'status') else 'DONE'}"
            }
        })
        ...
        await ws.send_json({
            "type": "phase",
            "data": {"phase": "complete", "status": "completed"}
        })
        
    except Exception as e:
        await ws.send_json({
            "type": "error",
            "data": f"Erreur lors de la re-vérification: {str(e)}"
        })
...
async def handle_force_repair(data: dict, ws: WebSocket, db: Session):
    ...
    last_user = db.query(Message).filter(
        Message.conversation_id == conversation_id,
        Message.role == "user"
    ).order_by(Message.created_at.desc()).first()
    ...
    await ws.send_json({
        "type": "phase",
        "data": {"phase": "repair", "status": "running", "message": "Réparation forcée en cours..."}
    })
...
Réponds directement avec la version améliorée."""
        ...
        result = await workflow_engine.run(
            ...
            history=history_list,
            websocket=ws,
            skip_spec=True,  # Skip spec pour repair, on va direct à l'exécution
        )
        ...
        # Note: Le message "complete" est envoyé par workflow_engine
        
    except Exception as e:
        await ws.send_json({
            "type": "error",
            "data": f"Erreur lors de la réparation: {str(e)}"
        })
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                await ws.send_json({
                    "type": "error",
                    "data": f"Erreur Ollama: HTTP {response.status_code}"
                })
                
    except httpx.TimeoutException:
        await ws.send_json({
            "type": "error",
            "data": "Timeout: Ollama ne répond pas"
        })
    except Exception as e:
        await ws.send_json({
            "type": "error",
