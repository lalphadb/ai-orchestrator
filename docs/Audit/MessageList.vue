  ...
  <div ref="container" class="flex-1 overflow-y-auto p-4 space-y-4">
    ...
    >
      ...
      >
        ...
        <!-- Special: Models Display (détection améliorée) -->
        <ModelsDisplay 
...
/**
 * Détection améliorée des listes de modèles
 * Reconnaît le format JSON brut affiché dans les screenshots
 */
function isModelsList(content) {
  ...
  // Pattern 1: Format JSON avec "name", "size", "available"
  const jsonModelPattern = /\{\s*"name":\s*"[^"]+"\s*,\s*"size":\s*\d+/g
  const jsonMatches = cleaned.match(jsonModelPattern)
  if (jsonMatches && jsonMatches.length >= 3) {
    return true
  }
  
  // Pattern 2: Plusieurs noms de modèles connus
  const knownModels = [
    'llama', 'qwen', 'deepseek', 'gemini', 'kimi', 
    'nomic', 'bge', 'mxbai', 'safeguard', 'vision',
    'instruct', 'coder', 'embed'
  ]
  ...
  // Si on trouve beaucoup de références à des modèles
  return foundCount >= 5
}

/**
 * Parse les modèles depuis le contenu brut
 */
function parseModels(content) {
  ...
  // Pattern pour extraire les objets modèle complets
  // Format: { "name": "...", "size": ..., "modified_at": "...", "available": true/false }
  const fullPattern = /\{\s*"name":\s*"([^"]+)"[^}]*?"size":\s*(\d+)[^}]*?"modified_at":\s*"([^"]*)"[^}]*?"available":\s*(true|false)\s*\}/gi
 ...
 * CORRIGÉ: Préserve la lisibilité
 */
function renderContent(content) {
  if (!content) return ''
  
  // Si c'est une liste de modèles, ne pas render (le composant s'en charge)
  if (isModelsList(content)) {
    return ''
  }
